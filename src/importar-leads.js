document.addEventListener('DOMContentLoaded', () => {
  ...
  enviarButton.addEventListener('click', () => {
    ...
    const extensaoPermitida = ['txt', 'csv', 'xlsx'];
    const fileName = arquivo.name;
    const extension = fileName.split('.').pop().toLowerCase();
    if (!extensaoPermitida.includes(extension)) {
      alert('Formato de arquivo não permitido. Utilize txt, csv ou xlsx.');
      return;
    }
    ...
    reader.onload = function (e) {
      ...
      const webhookUrl = 'https://n8n-other.neuralbase.com.br/webhook/importar-leads';
